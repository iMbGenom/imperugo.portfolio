...
(function() {
	...
	var Portfolio = Component.extend({
			...
			$.get(href, function(response) {
				...
				imagesLoaded(container.get(0), function() {
					...
					if ($.fn.nivoLightbox !== undefined) {
						$('a[data-lightbox="nivoLightbox"]', container).nivoLightbox({
		...
		_loadAjaxContent: function(e) {
			...
			if (!elm.hasClass('inprocess')) {
				...
				panel.html('<div class="line-load-page">Loading...</div>');
	...
	var Sticky = Component.extend({
		init: function(element, options) {
			this._super(element, options);
			this.containment = this.options.containment ? $(this.options.containment) : this.element.parent();
		...
		handleScroll: function() {
			var originalOffset			= this.wrapper.offset(),
					containmentOffset		= this.containment.offset(),
					containmentHeight		= this.containment.height();

			if ($(window).scrollTop() > originalOffset.top) {
				if ($(window).scrollTop() + this.element.height() >= containmentOffset.top + containmentHeight) {
